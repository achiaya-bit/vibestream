...
export function PlaylistCard({ playlist }: { playlist: Playlist }) {
  return (
    <motion.div whileHover={{ y: -6 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
      <Link to="/playlist/$id" params={{ id: playlist.id }} className="group block glass rounded-2xl p-4 relative">
        ...
        <p className="mt-1 text-xs text-muted-foreground">{playlist.songCount} tracks · {playlist.curator}</p>
      </Link>
    </motion.div>
  );
}
