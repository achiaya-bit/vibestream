  ...
  if (isError) {
    return (
      <p className="text-center py-20 text-muted-foreground">
        ...
        <code className="text-[var(--neon-cyan)]">npm run dev</code> in vibestream-api.
      </p>
    );
  }
  ...
  return (
    <div className="animate-fade-up">
      <motion.div
        ...
        className="relative overflow-hidden rounded-3xl glass-strong p-6 md:p-10 mb-10"
      >
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="mt-3 text-muted-foreground max-w-xl">
              Your queue, recommendations and trending tracks — all curated for tonight.
            </p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
          {playlists.slice(0, 8).map((p) => (
            <Link
              ...
              to="/playlist/$id"
              params={{ id: p.id }}
              className="group glass rounded-xl flex items-center gap-3 pr-4 overflow-hidden hover:bg-white/10 transition"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {artists.map((a) => (
            <Link
              ...
              to="/artist/$id"
              params={{ id: a.id }}
              className="group glass rounded-2xl p-4 text-center hover:bg-white/10 transition"
            >
              <div
                className={`mx-auto h-32 w-32 rounded-full bg-gradient-to-br ${a.image} shadow-glow group-hover:scale-105 transition`}
              />
